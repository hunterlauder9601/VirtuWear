...
export const metadata = {
  title: "Men's Products - VirtuWear",
};
...
export default async function MensProducts({
  ...
  const products = await getClothingItems("men", group, currentPage, pageSize);
  ...
  return (
    <div className="flex max-h-fit min-h-[calc(100vh-65px)] w-full flex-col items-center justify-start  bg-base-100 text-white">
      <div className="mt-[calc(64px+10vh)] flex h-full w-full max-w-6xl flex-col items-center justify-start p-4">
        <h1 className="mb-12 -skew-x-6 bg-gradient-to-r from-primary to-accent p-2 text-3xl font-bold tracking-widest text-base-100">
          MEN&apos;S PRODUCTS
        </h1>
        <div className="btn-group btn-group-vertical mb-8 lg:btn-group-horizontal">
          {categories.map((category) => (
            <Link
              href={`/products/men?group=${category}`}
              key={category}
